...
torch.manual_seed(0)
np.random.seed(0)
model_id_name = "gpt2-large"

# find a better way to abstract the class
class LLAMA2PPL:
    def __init__(self, model_device="cuda:0", t5_device='cuda:1', model_id="../aila-llama2-13b-hf"):
        self.device = model_device
        self.t5_device = t5_device
        self.model_id = model_id
        self.model = AutoModelForCausalLM.from_pretrained(model_id, load_in_8bit=True) # .to(model_device)
        ...
        self.t5_model = transformers.AutoModelForSeq2SeqLM.from_pretrained("t5-large").to(t5_device).half()
        self.t5_tokenizer = T5Tokenizer.from_pretrained("t5-large", legacy=True, model_max_length=512)
    ...
    def unmasker(self, text, num_of_masks):
        ...
        for key in tokens:
            tokens[key] = tokens[key].to(self.t5_device)
...
###############################################
    ...
    def call_1_1(self, sentence, chunk_value):
        ...
        print(f"probability for {'AI' if label == 0 else 'Human'}:", "{:.2f}%".format(mean_prob))
